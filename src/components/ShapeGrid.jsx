...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    let currentHoverFillColor = hoverFillColor;
    let currentBorderWidth = lightBorderWidth;
    ...
    const updateThemeColors = () => {
      ...
      currentHoverFillColor =
        hoverFillColor ||
        styles.getPropertyValue('--grid-hover').trim() ||
        '#222';

      currentBorderWidth = isDark
        ? darkBorderWidth
        : lightBorderWidth;
    };
    ...
    const updateAnimation = () => {
      // FIX: speed adalah angka, bukan function
    ...
    const updateCellOpacities = () => {
      ...
      for (
        const [key, opacity] of cellOpacities.current
      ) {
        const target = targets.get(key) || 0;
        const next =
          opacity + (target - opacity) * 0.15;
    ...
    const handleMouseMove = event => {
      ...
      if (isHex) {
        ...
        const col =
          Math.round(
            adjustedX / squareSize
          );

        const row =
          Math.round(
            adjustedY / squareSize
          );
    ...
    // Detect perubahan class .dark pada <html>

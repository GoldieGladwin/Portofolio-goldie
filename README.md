...
* **RLS Enabled:** Diaktifkan penuh pada tabel `proyek`.
* **SELECT Policy:** `"Enable read access for all users"` dengan rule `FOR SELECT USING (true)` sehingga data dapat dibaca secara aman oleh frontend Next.js tanpa membocorkan akses mutasi (Create/Update/Delete).
